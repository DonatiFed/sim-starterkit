...
def decide_daily_special(state, memory) -> str | None:
    ...
    weekend_or_premium = (state.day_of_week in ("Friday", "Saturday")
                         or state.weather_today == "sunny")

    # If we have ingredients expiring, prioritize burning them on slow days
    expiring_best: tuple[str, float] | None = None
    for dish in active:
        ...
        if expiring_best is None or score > expiring_best[1]:
            expiring_best = (dish, score)

    # On slow days with expiring inventory: push expiring
    if expiring_best and expiring_best[1] > 0.5 and not weekend_or_premium:
        return expiring_best[0]

    # On busy/premium days: pick highest-margin dish to push revenue
    if weekend_or_premium:
        by_margin = sorted(
    ...
    # Otherwise: expiring if any
    if expiring_best and expiring_best[1] > 0.3:
        return expiring_best[0]

    # Fallback: highest-margin available
    by_margin = sorted(
        [d for d in active if d in RECIPES],
        key=lambda d: -RECIPES[d][0],
    )
