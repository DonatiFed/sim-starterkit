...
def plan_orders(state, memory, projection: dict, runway: dict) -> list[dict]:
    ...
    # Weekend surge pre-order: on Wed/Thu/Fri, pre-buy for Fri/Sat peak.
    # Friday demand ~245, Saturday ~235 — double the weekday burn.
    if state.day_of_week in ("Wednesday", "Thursday") and state.days_remaining > 4:
        for ing in ["Flour", "Mozzarella", "Tomato Sauce", "Fresh Pasta"]:
            if ing in [n[0] for n in needs]:
                continue
            on_hand = state.inventory.get(ing)
            ...
            pending_kg = state.pending_by_ingredient.get(ing, 0.0)
            daily_use = _compute_daily_use(state, memory, ing)
            # Need 2.5× daily for Fri+Sat surge + buffer
            target = daily_use * 4
            gap = max(0.0, target - on_hand_kg - pending_kg)
            if gap > 3.0:
                needs.append((ing, gap * over_order, "weekend_prep"))

    # Generic defensive mode: if an unknown alert fired, build a 5-day buffer
    # of the bread-and-butter ingredients. Cheap insurance against surprises.
    if memory.scen_unknown_alert and state.day <= 4:
        for ing in ["Flour", "Mozzarella", "Tomato Sauce", "Fresh Pasta", "Cream"]:
            if ing in [n[0] for n in needs]:
                continue
            on_hand = state.inventory.get(ing)
            on_hand_kg = on_hand.total_kg if on_hand else 0.0
            ...
            gap = max(0.0, target - on_hand_kg - pending_kg)
            if gap > 2.0:
                needs.append((ing, gap * over_order, "unknown_buffer"))

    # Sort by urgency
    rank = {"critical": 0, "high": 1, "crisis_buffer": 2, "unknown_buffer": 3,
            "weekend_prep": 4, "medium": 5, "topup": 6}
